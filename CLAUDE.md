...
- **HyperFrames**: CLI via `npx --yes hyperframes` (cache do npx aquecido pelo `setup.sh`; primeira execução precisa do registry pelo proxy). Render local usa o `headless_shell` do Playwright pelo symlink `/usr/local/bin/hf-headless-shell`, que o `setup.sh` cria. As variáveis `HYPERFRAMES_BROWSER_PATH` e `HYPERFRAMES_SKIP_SKILLS=1` vivem em `.claude/settings.json` (o shell da ferramenta é não interativo e não lê `.bashrc`; `settings.json` vale a partir da sessão seguinte).
...
| `cdn.jsdelivr.net` | **403, liberar** | templates e blocos do registry do HyperFrames carregam o GSAP de lá; sem isso o render é bloqueado (`sub_timeline_script_failure`) e só composição com GSAP local funciona |
| `unpkg.com`, `esm.sh`, `cdnjs.cloudflare.com` | 403 | outros CDNs que blocos do registry podem citar; liberar conforme aparecerem |
| `hyperframes.heygen.com` | 403 | docs e render em nuvem do HyperFrames; o render local não precisa |
| `fonts.googleapis.com`, `fonts.gstatic.com` | 403 | sem fontes por rede no render; embutir em `assets/fonts/` |
...
- **HyperFrames renderiza local com o `headless_shell` do Playwright, apontado por `HYPERFRAMES_BROWSER_PATH`.** O `npx hyperframes browser ensure` baixaria um Chrome próprio, fora da allowlist. O `setup.sh` cria `/usr/local/bin/hf-headless-shell` apontando para o binário do Playwright e o `.claude/settings.json` exporta a variável. `hyperframes render` recebe o **diretório** do projeto, não o `index.html`. Validado em 18/set/2026: projeto em branco em retrato, 300 frames 1080x1920, 14 s. O `doctor` às vezes reprova o ffmpeg de forma intermitente; o que vale é o render do `validate.sh`.
- **Templates e blocos do HyperFrames carregam o GSAP de `cdn.jsdelivr.net`.** Com o CDN fora da allowlist o render captura tudo e é bloqueado no fim com `sub_timeline_script_failure`. Contorno para teste: copiar `skills/talking-head-recut/assets/vendor/gsap.min.js` do clone do hyperframes para `vendor/` do projeto e trocar o `src` do script. Para produção, liberar `cdn.jsdelivr.net` (mais de mil arquivos do clone citam jsdelivr ou unpkg). O registry de blocos (`hyperframes add`) vive em `raw.githubusercontent.com`, também bloqueado.
- **`hyperframes init` consulta o GitHub para checar skills** e trava sem `raw.githubusercontent.com`; `HYPERFRAMES_SKIP_SKILLS=1` (no `settings.json`) pula isso. As skills já vêm do clone local.
