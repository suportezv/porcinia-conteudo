...
- **OpenAI e Gemini (imagem)**: `OPENAI_API_KEY` e `GEMINI_API_KEY` **ausentes** neste environment, e `api.openai.com` e `generativelanguage.googleapis.com` respondem **403 no CONNECT** (fora da allowlist). `scripts/gera_imagem.py` está no repo mas **não funciona aqui até cadastrar as duas chaves e liberar os dois hosts**. Depois de cadastrar, abrir sessão nova (env var só entra na criação do container).
- **Drive**: conector oficial conectado. Material da marca já localizado (buscar por nome no conector; só o logo é público):
  ...
  - **Upload para o Drive não funciona deste container**: `www.googleapis.com` responde 403 no CONNECT. `scripts/sobe_para_drive.py` fica à espera da allowlist.
...
| `api.openai.com` | **403, liberar** | necessário para `gera_imagem.py openai` |
| `generativelanguage.googleapis.com` | **403, liberar** | necessário para `gera_imagem.py gemini`; liberar `www.googleapis.com` **não** cobre este subdomínio |
| `www.googleapis.com` | **403, liberar** | necessário para `sobe_para_drive.py` |
